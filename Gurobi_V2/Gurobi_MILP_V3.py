...
# Parameters
load_demand = 250  # Total hourly load demand (kW)
PowerTurbine = [5, 10, 15]  # Wind turbine capacities (kW)
PowerPV = [3, 6, 9]  # Solar PV capacities (kW)

# Turbine cost $100/kWh, PV cost $50/kWh
costTurbine = [100, 180, 360]  # Cost per kWh for wind turbines
costPV = [50, 90, 130]  # Cost per kWh for solar PV
costgrid = 55  # Cost per kWh from the grid

# Define maximum quantities
max_turbines = 6  # Maximum number of turbines
max_pv = 15  # Maximum number of PV systems
...
for _, row in power_data.iterrows():
    ...
    model.setObjective(
        sum(costTurbine[i] * turbine[i] for i in range(3)) +
        sum(costPV[i] * PV[i] for i in range(3)) +
        costgrid * grid_energy,
        GRB.MINIMIZE
    )

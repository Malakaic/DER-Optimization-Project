...
def wind_function_wind_solar(lat_2, lon_2):
    # Check if latitude and longitude are valid
    if not (isinstance(lat_2, (int, float)) and isinstance(lon_2, (int, float))):
        raise ValueError("Latitude and longitude must be numeric.")

    # Cache key
    cache_key = f"{lat_2},{lon_2}"
    ...
    folder_name = "Environmental Data V3"
    project_dir = os.getcwd()
    folder_path = os.path.join(project_dir, folder_name)
    os.makedirs(folder_path, exist_ok=True)

    # Define file paths
    wind_speed_file = os.path.join(folder_path, "wind_data_import_wind_and_solar.csv")
    output_file = os.path.join(folder_path, "wind_power_output_wind_and_solar.csv")
